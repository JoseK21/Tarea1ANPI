...
%}
function [x0, itr] = sne_ud_2 (f, x0, tol, graf = 1)
  pkg load symbolic;
  try # Se utiliza try para atrapar errores de sintaxis
    ...
    while (abs(ff(x0)) >= tol)  # Se itera hasta que se cumpla una condicion de parada.    
      x1 = x0 - ff(x0) / fd(x0 - 0.5 * ff(x0) / fd(x0));  # Sucesion de x
    ...
    if (graf==1)  #Se grafica si el valor de graf es 1.
       ...
       title("Metodo de Frontini and Sormani, 0zban", "fontsize", 20); # Titulo del grafico.
