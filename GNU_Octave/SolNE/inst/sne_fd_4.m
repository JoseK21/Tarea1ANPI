...
Tomado del Artículo Científico: A family of Kurchatov-type methods and its stability
Pag.1
Ecuación (2)
Ejemplo : [xaprox, iter] = sne_fd_4("x^2 - 18",0.3,0.0001, 1)
...
%}
function [x0, itr] = sne_fd_4 (f, x0, tol, graf = 1)
  pkg load symbolic;
  try # Se utiliza try para atrapar errores de sintaxis
    ...
    while (abs(ff(x0)) >= tol)  # Se itera hasta que se cumpla una condicion de parada.
      ...
      wk = x0 + ff(x0);  # Sucesión de w
      fxkwk =  (ff(x0) - ff(wk)) / (x0 - wk); # Sucesión de f(xk, wk)
      x1 = x0 - ff(x0) / fxkwk;  # Sucesión de x   
    ...
    if (graf==1)  #Se grafica si el valor de graf es 1.
       ...
       title("Metodo Steffensen’s scheme", "fontsize", 20); # Titulo del grafico.
